...
class JL:
	...
	def __init__(self, path_json, n_lfs, n_features, feature_model = 'nn', n_hidden = 512):
		...
		assert type(n_features) == int or type(n_features) == float
		assert type(n_hidden) == int or type(n_hidden) == float
		assert feature_model == 'lr' or feature_model == 'nn' or feature_model == 'rcnn'
		...
		elif self.feature_based_model =='rcnn':
			print("Calling FastRCNN")
			backbone = torchvision.models.mobilenet_v2(pretrained=True).features
			backbone.out_channels = 1280
			anchor_generator = AnchorGenerator(sizes=((32, 64, 128, 256, 512),),aspect_ratios=((0.5, 1.0, 2.0),))
			roi_pooler = torchvision.ops.MultiScaleRoIAlign(featmap_names=['0'], output_size=7, sampling_ratio=2)
			self.feature_model = faster_rcnn.FasterRCNN(backbone, num_classes=21, rpn_anchor_generator=anchor_generator, box_roi_pool=roi_pooler)
		else:
	...
	def fit_and_predict_proba(self, path_L, path_U, path_V, path_T, loss_func_mask, batch_size, lr_fm, lr_gm, use_accuracy_score, path_log = None, return_gm = False, n_epochs = 100, start_len = 7,\
	 stop_len = 10, is_qt = True, is_qc = True, qt = 0.9, qc = 0.85, metric_avg = 'binary'):
		...
		assert type(return_gm) == bool
		assert (type(loss_func_mask) == list) and len(loss_func_mask) == 7
		assert type(batch_size) == int or type(batch_size) == float
		assert type(lr_fm) == int or type(lr_fm) == float
		assert type(lr_gm) == int or type(lr_gm) == float
		assert type(use_accuracy_score) == bool
		assert type(n_epochs) == int or type(n_epochs) == float
		assert type(start_len) == int or type(start_len) ==float
		assert type(stop_len) == int or type(stop_len) == float
		assert type(is_qt) == bool and type(is_qc) == bool
		...
		print(self.n_features,x_sup.shape[1])
				...
				for _, sample in enumerate(loader):
					...
					if(loss_func_mask[0]):
						if len(supervised_indices) > 0:
							print(sample[0][supervised_indices].shape)
							print(sample[1][supervised_indices]["boxes"])
							loss_1 = supervised_criterion(self.feature_model(sample[0][supervised_indices],sample[1][supervised_indices]), sample[1][supervised_indices])
						else:
							loss_1 = 0
					else:
						loss_1 = 0
