...
class Cage:
	...
	def fit_and_predict_proba(self, path_pkl, path_test = None, path_log = None, qt = 0.9, qc = 0.85, metric_avg = ['weighted'], n_epochs = 100, lr = 0.03):
		'''
		Args:
		...
		# data = [arr for arr in data if not has_nan(arr) and not any(x is None for x in np.ravel(arr))]
		# print("data",data[1])
		import numpy as np

		# Assuming 'data' is your numpy array
		# data_list = data[1].tolist()  # Convert to a list
		# for i in range(len(data_list)):
		# 	for j in range(len(data_list[i])):
		# 		if data_list[i][j] == 21:
		# 			data_list[i][j] = None
		# data[1] = np.array(data_list)  # Convert back to a NumPy array
		# print(data[1])
		...
		s[s > 0.833] = 0.833 # clip s
		s[s < 0.001] = 0.001 # clip s
		print("S after clipping: ",s)
		...
		# print(self.pi)
		optimizer = optim.AdamW([self.theta, self.pi], lr=lr)
	...
	def fit_and_predict(self, path_pkl, path_test = None, path_log = None, qt = 0.9, qc = 0.85, metric_avg = ['weighted'], n_epochs = 100, lr = 1000, need_strings = False):
		'''
		Args:
			path_pkl: Path to pickle file of input data in standard format
			path_test: Path to the pickle file containing test data in standard format
